...
Generator.prototype.attach = function (options) {
  ...
  **/
  ...
  // object to reverse special characters (for example, it is needed for building valid urls for google maps)
  // uncomment, if other characters are needed
  ...
  _.extend(router, {
    ...
    /* decode some reserved special characters */
    regExprForSpecChars: new RegExp(
        _.map(
            specialCharacters,
            function(v, k) {
              return '(' + k + ')';
    ...
    decodeUrlSegment: function(urlSegment) {
      return urlSegment.replace(router.regExprForSpecChars, function(match) {
        return specialCharacters[match.toLowerCase()];
      });
    },
          ...
          if (pConfig && pConfig.kind === 'rest') {
            ...
            if (pval) {
              ...
              if (_.isObject(pval)) {
                ...
                uri.pathname += '/' + router.decodeUrlSegment(encodeURIComponent(pConfig.regex.replace(/\(.*\)/, segment)));

              } else {
                uri.pathname += '/' + router.decodeUrlSegment(encodeURIComponent(pConfig.regex.replace(/\(.*\)/, pval)));
              }
            }
          }
        }
